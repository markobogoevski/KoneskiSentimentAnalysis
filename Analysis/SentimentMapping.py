...
if __name__ == "__main__":
    ...
    # dict_path = "dictionary_lemma.pkl"
    ...
    for _, row in songs_df.iterrows():
        ...
        # zborovi_mk = [word for word in zborovi_eng if word not in stop_words]
        ...
        V_mean_eng = sum([v for v in vector_V_eng if v != -1]) / vad_len_eng
        V_mean_mk = sum([v for v in vector_V_mk if v != -1]) / vad_len_mk

        A_mean_eng = sum([v for v in vector_A_eng if v != -1]) / vad_len_eng
        A_mean_mk = sum([v for v in vector_A_mk if v != -1]) / vad_len_mk

        D_mean_eng = sum([v for v in vector_D_eng if v != -1]) / vad_len_eng
        D_mean_mk = sum([v for v in vector_D_mk if v != -1]) / vad_len_mk
    ...
    df_eng.to_csv("Data/SentimentEng.csv", encoding='utf-8', index=False)
    # With lemma dict
    #df_eng.to_csv("Data/SentimentEngLemma.csv", encoding='utf-8', index=False)

    df_mk = pd.DataFrame(final_df_list_mk)
    df_mk.to_csv("Data/SentimentMk.csv", encoding='utf-8', index=False)
    # With lemma dict
    #df_mk.to_csv("Data/SentimentMkLemma.csv", encoding='utf-8', index=False)
