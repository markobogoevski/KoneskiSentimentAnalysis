...
def split_text_into_songs(full_corpus, start_word, end_word):
    ...
    for tome in tomes[1:]:
        ...
        tome_year = tome_split[2].split()[0]
        ...
        if len(pod_tom_split) == 1:
            tome_texts.append({'tome_name': tome_name, 'pod_tome_name': tome_name, 'text': tome_text_f,
                               'tome_year': tome_year, 'tome_num': tome_num, 'pod_tome_num': 1})
        else:
            counter = 1
        ...
        for i in range(len(text_in)):
            ...
            if str.isupper(line) or (str.isnumeric(line.split('.')[0]) and line.endswith(".")):
                if not found_start:
                    ...
                    if end - start > 1:
                        strofi.append(text_in[start:end])
                    start = end
        strofi.append(text_in[start:])
        tome_text['strofi'] = strofi
...
if __name__ == "__main__":
    ...
    # {'tome_name': tome_name, 'pod_tome_name': pod_tom_ime, 'text': pod_tom_tekst,
    # 'tome_year': tome_year, 'tome_num': tome_num, 'pod_tome_num': counter})
    final_dict = {'Godina': [], 'Zbirka': [], 'Zbirka_broj': [], 'Podzbirka': [], 'Podzbirka_broj': [],
                  'Strofa_Broj': [], 'Strofa': [], 'Stih_Broj': [], 'Stih': []}

    for item in dictionary_tome_1:
        strofi = item['strofi']
        counter_strofa = 1
        for strofa in strofi:
            counter_stih = 1
            for stih in strofa:
                final_dict['Godina'].append(item['tome_year'])
                ...
                final_dict['Strofa_Broj'].append(str(counter_strofa))
                final_dict['Strofa'].append('\n'.join(strofa))
